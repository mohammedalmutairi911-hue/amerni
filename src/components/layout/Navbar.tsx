  ...
  return (
    <nav className="fixed top-0 inset-x-0 z-50 bg-gray-50/90 backdrop-blur border-b border-gray-200">
      <div className="max-w-6xl mx-auto px-4 h-14 flex items-center justify-between">
        ...
        <button onClick={toggleTheme} className="w-8 h-8 rounded-lg flex items-center justify-center text-gray-500 hover:text-gray-900 hover:bg-gray-100 transition-colors text-sm">
          {theme === 'dark' ? '☀️' : '🌙'}
        </button>
        ...
        <div className="flex items-center gap-3">
          {user && profile ? (
            <div className="relative">
              ...
              {dropOpen && (
                <div className="absolute left-0 top-full mt-2 w-52 bg-white border border-gray-200 rounded-xl shadow-2xl overflow-hidden z-50">
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => openAuth('signup')}
                className="text-sm bg-primary-500 text-gray-900 font-bold px-4 py-1.5 rounded-lg hover:bg-primary-400 transition-colors">
                سجّل
              </button>
            </div>
          )}
