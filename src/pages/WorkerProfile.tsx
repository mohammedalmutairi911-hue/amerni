  ...
  return (
    <div className="min-h-screen bg-slate-50 pt-14">
      <div className="max-w-lg mx-auto px-4 py-8">
        <button onClick={() => navigate('browse')} className="flex items-center gap-1.5 text-slate-500 hover:text-slate-900 text-sm mb-6 transition-colors">
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center gap-2">
                <div className="flex items-center gap-0.5">
                  {[1,2,3,4,5].map(s => <Star key={s} size={14} className={s <= Math.round(worker.rating || 0) ? 'text-primary-400 fill-primary-400' : 'text-slate-300'} />)}
                </div>
                <span className="text-sm text-slate-900 font-bold">{worker.rating ? worker.rating.toFixed(1) : '—'}</span>
                <span className="text-xs text-slate-400">({worker.total_tasks || 0} طلب مكتمل)</span>
              </div>
            </div>
            <div className={`w-3 h-3 rounded-full flex-shrink-0 mt-1 ${worker.is_online ? 'bg-secondary-500' : 'bg-zinc-600'}`} />
          </div>
          {worker.bio && <p className="text-sm text-slate-500 leading-relaxed">{worker.bio}</p>}
        </div>

        {/* Skills */}
        <div className="bg-white border border-slate-200 rounded-2xl p-5 mb-4">
          <h3 className="font-semibold mb-3 text-sm text-slate-500">المهارات</h3>
        ...
        {worker.schedule && Object.values(worker.schedule).some((s: any) => s.active) && (
          <div className="bg-white border border-slate-200 rounded-2xl p-5 mb-6">
            <h3 className="font-semibold mb-3 text-sm text-slate-500">أوقات التوفر</h3>
