    ...
    <div className="min-h-screen bg-slate-50 pt-14 px-4 pb-10">
      <div className="max-w-lg mx-auto">
        <div className="text-center mb-8">
          ...
          <p className="text-slate-400 text-sm">أكمل الخطوات عشان تبدأ تقبل طلبات</p>
        </div>
        ...
        <div className="flex items-center justify-center gap-1 mb-8 flex-wrap">
          {STEP_LABELS.map((label, i) => (
            <div key={i} className="flex items-center gap-1">
              ...
              {i < 3 && <div className={`w-3 h-0.5 ${i + 1 < step ? 'bg-secondary-500' : 'bg-slate-100'}`} />}
            </div>
          ))}
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-6">
          ...
          {step === 1 && (
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-xs text-slate-400 mb-1.5">نبذة عنك (اختياري)</label>
          ...
          {step === 2 && (
            <div className="space-y-5">
              ...
              <div>
                <label className="block text-xs text-slate-400 mb-2">صورة الهوية (اختياري)</label>
                {idPreview ? (
                  <div className="relative">
                    <img src={idPreview} alt="ID" className="w-full h-36 object-cover rounded-xl border border-slate-300" />
                    <label className="absolute bottom-2 left-2 bg-slate-900/60 text-xs text-slate-900 px-3 py-1.5 rounded-lg cursor-pointer flex items-center gap-1">
                      <Camera size={12} /> تغيير
                      <input type="file" accept="image/*" onChange={handleIdUpload} className="hidden" />
                    </label>
                  </div>
                ) : (
                  <label className="flex flex-col items-center justify-center gap-3 bg-white border-2 border-dashed border-slate-300 rounded-xl p-6 cursor-pointer hover:border-primary-500/50 transition-colors">
                    <Upload size={22} className="text-slate-400" />
                    <span className="text-sm text-slate-400">اضغط لرفع صورة الهوية (اختياري)</span>
                    <input type="file" accept="image/*" onChange={handleIdUpload} className="hidden" />
                  </label>
                )}
              </div>
              {!verified && form.id_number.length === 10 && (
                <button onClick={verifyId} disabled={verifying}
                  className="w-full bg-slate-100 hover:bg-slate-200 text-slate-900 py-2.5 rounded-xl text-sm font-medium transition-colors flex items-center justify-center gap-2 disabled:opacity-50">
          ...
          {step === 3 && (
            <div>
              ...
              <p className="text-slate-400 text-sm mb-5">اختر المهارات اللي تقدر تنجز فيها طلبات</p>
              <div className="grid grid-cols-2 gap-2">
                {SKILLS.map(s => (
                  <button key={s} onClick={() => toggleSkill(s)}
                    className={`text-right px-4 py-3 rounded-xl border text-sm transition-all ${
                      form.skills.includes(s) ? 'border-primary-500 bg-primary-500/10 text-primary-300' : 'border-slate-200 text-slate-500 hover:border-slate-300'
                    }`}>
                    {s}
                  </button>
                ))}
          ...
          {step === 4 && (
            <div>
              ...
              <p className="text-slate-400 text-sm mb-5">حدد الأيام والأوقات اللي تكون فيها متاح</p>
              <div className="space-y-2">
                {DAYS.map(d => (
                  <div key={d} className={`rounded-xl border p-3 transition-all ${form.schedule[d].active ? 'border-primary-500/30 bg-primary-500/5' : 'border-slate-200'}`}>
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2">
                        <Clock size={13} className={form.schedule[d].active ? 'text-primary-400' : 'text-slate-400'} />
                        <span className={`text-sm font-medium ${form.schedule[d].active ? 'text-slate-900' : 'text-slate-400'}`}>{d}</span>
                      </div>
                      <button onClick={() => toggleDay(d)}
                        className={`w-10 h-5 rounded-full transition-all relative ${form.schedule[d].active ? 'bg-primary-500' : 'bg-slate-200'}`}>
                    ...
                    {form.schedule[d].active && (
                      <div className="flex items-center gap-2 mt-2">
                        <input type="time" value={form.schedule[d].from} onChange={e => updateTime(d, 'from', e.target.value)}
                          className="bg-slate-100 border border-slate-300 rounded-lg px-2 py-1 text-xs text-slate-900 outline-none" />
          ...
          <div className="flex gap-3 mt-6">
            {step > 1 && (
              <button onClick={() => { setStep(s => s - 1); setError('') }}
                className="flex-1 border border-slate-300 text-slate-600 py-2.5 rounded-xl text-sm hover:border-slate-300 transition-colors">
                رجوع
              </button>
            )}
            {step < 4 ? (
