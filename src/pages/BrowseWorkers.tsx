        ...
        <div className="flex flex-col gap-3 mb-6">
          <div className="relative">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="ابحث باسم العامل أو المهارة..."
              className="w-full bg-[#111] border border-zinc-800 rounded-xl pr-10 pl-4 py-3 text-sm outline-none focus:border-primary-500/50 transition-colors" />
          </div>
          <div className="flex gap-2 overflow-x-auto pb-1">
            ...
            {SKILLS.map(s => (
              <button key={s} onClick={() => setSkillFilter(s)}
                className={`px-3 py-1.5 rounded-full text-xs font-medium whitespace-nowrap transition-all flex-shrink-0 ${skillFilter === s ? 'bg-primary-500 text-white' : 'bg-zinc-800 text-zinc-400 hover:bg-zinc-700'}`}>
                {s}
              </button>
            ))}
          </div>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {filtered.map(w => (
              <button key={w.id} onClick={() => setSelectedWorker(w)}
                className="bg-[#0d0d0d] border border-zinc-800 hover:border-primary-500/30 rounded-2xl p-5 text-right transition-all">
