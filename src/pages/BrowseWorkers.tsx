  ...
  if (selectedWorker) return (
    <div className="min-h-screen bg-gray-50 pt-14 px-4 py-8">
      <div className="max-w-lg mx-auto">
        <button onClick={() => setSelectedWorker(null)} className="flex items-center gap-1.5 text-gray-500 hover:text-gray-900 text-sm mb-6 transition-colors">
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-6 mb-4">
          <div className="flex items-start gap-4 mb-5">
            ...
            <div className="flex-1">
              ...
              <p className="text-gray-400 text-sm flex items-center gap-1"><MapPin size={12} /> {selectedWorker.city}</p>
              <div className="flex items-center gap-3 mt-2">
                <div className="flex items-center gap-1">
                  {[1,2,3,4,5].map(s => <Star key={s} size={13} className={s <= Math.round(selectedWorker.rating || 0) ? 'text-primary-400 fill-primary-400' : 'text-zinc-700'} />)}
                  <span className="text-sm text-gray-900 mr-1">{selectedWorker.rating ? selectedWorker.rating.toFixed(1) : '—'}</span>
                </div>
                <span className="text-xs text-gray-400">{selectedWorker.total_tasks || 0} طلب مكتمل</span>
              </div>
            </div>
            <div className={`w-3 h-3 rounded-full flex-shrink-0 mt-1 ${selectedWorker.is_online ? 'bg-secondary-500' : 'bg-gray-300'}`} />
          </div>

          {selectedWorker.bio && (
            <div className="border-t border-gray-200 pt-4 mb-4">
              <p className="text-sm text-gray-500 leading-relaxed">{selectedWorker.bio}</p>
            </div>
          )}

          {/* Skills */}
          <div>
            <p className="text-xs text-gray-400 mb-2 font-medium">المهارات</p>
        ...
        <button onClick={() => { if (user) navigate('dashboard'); else navigate('landing') }}
          className="w-full bg-primary-500 text-gray-900 font-bold py-4 rounded-2xl text-base hover:bg-primary-400 transition-colors flex items-center justify-center gap-2">
    ...
    <div className="min-h-screen bg-gray-50 pt-14">
      <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-400">اختار الشخص المناسب مباشرة</p>
        </div>
        ...
        <div className="flex flex-col gap-3 mb-6">
          <div className="relative">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="ابحث باسم العامل أو المهارة..."
              className="w-full bg-white border border-gray-200 rounded-xl pr-10 pl-4 py-3 text-sm outline-none focus:border-primary-500/50 transition-colors" />
          </div>
          <div className="flex gap-2 overflow-x-auto pb-1">
            ...
            {SKILLS.map(s => (
              <button key={s} onClick={() => setSkillFilter(s)}
                className={`px-3 py-1.5 rounded-full text-xs font-medium whitespace-nowrap transition-all flex-shrink-0 ${skillFilter === s ? 'bg-primary-500 text-gray-900' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'}`}>
                {s}
              </button>
            ))}
          </div>
          <div className="flex gap-2 overflow-x-auto pb-1">
            {CITIES.map(c => (
              <button key={c} onClick={() => setCityFilter(c)}
                className={`px-3 py-1.5 rounded-full text-xs font-medium whitespace-nowrap transition-all flex-shrink-0 ${cityFilter === c ? 'bg-gray-700 text-white' : 'bg-gray-100 text-gray-400 hover:bg-gray-100'}`}>
                {c}
              </button>
            ))}
          </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-20">
            <p className="text-gray-400 mb-3">ما في عمال بهذا الفلتر</p>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {filtered.map(w => (
              <button key={w.id} onClick={() => setSelectedWorker(w)}
                className="bg-white border border-gray-200 hover:border-primary-500/30 rounded-2xl p-5 text-right transition-all">
                <div className="flex items-start gap-3 mb-4">
                  <div className="relative flex-shrink-0">
                    <img src={getAvatar(w.full_name)} className="w-12 h-12 rounded-xl" alt="" />
                    <div className={`absolute -bottom-1 -left-1 w-3.5 h-3.5 rounded-full border-2 border-white ${w.is_online ? 'bg-secondary-500' : 'bg-gray-300'}`} />
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-400 flex items-center gap-1"><MapPin size={10} /> {w.city}</p>
                  </div>
                </div>

                <div className="flex items-center gap-2 mb-3">
                  <div className="flex items-center gap-0.5">
                    {[1,2,3,4,5].map(s => <Star key={s} size={11} className={s <= Math.round(w.rating || 0) ? 'text-primary-400 fill-primary-400' : 'text-zinc-700'} />)}
                  </div>
                  <span className="text-xs text-gray-500">{w.rating ? w.rating.toFixed(1) : '—'}</span>
                  <span className="text-xs text-gray-400">({w.total_tasks || 0})</span>
                </div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  ...
                  {(w.skills || []).length > 2 && <span className="text-xs text-gray-400">+{(w.skills || []).length - 2}</span>}
                </div>
              </button>
            ))}
          </div>
