  ...
  if (!workerProfile?.is_approved) return (
    <div className="min-h-screen bg-slate-50 pt-14 flex items-center justify-center px-4">
      <div className="max-w-sm text-center bg-white border border-slate-200 rounded-2xl p-10">
        ...
        <p className="text-slate-400 text-sm leading-relaxed">فريق أمرني راح يراجع بياناتك ويوافق عليك قريباً. راح تجي لك إشعار فور الموافقة.</p>
      </div>
    </div>
  )
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-slate-900/80 backdrop-blur-sm p-0 sm:p-4">
      <div className="w-full max-w-sm bg-slate-50 border border-slate-200 rounded-t-2xl sm:rounded-2xl flex flex-col max-h-[85vh] sm:max-h-[80vh]">
        ...
        <div className="p-5 pb-3 flex-shrink-0">
          ...
          <p className="text-slate-500 text-xs text-center">قبل ما تقبل الطلب، يرجى الموافقة على الشروط التالية:</p>
        </div>
        ...
        <div className="overflow-y-auto flex-1 px-5 pb-3 space-y-3">
          <div className="bg-primary-500/10 border border-primary-500/20 rounded-xl p-3 space-y-1">
            <p className="text-sm text-slate-900 font-semibold">الطلب: {pendingTask.title}</p>
            {pendingTask.price_suggested && (
              <p className="text-sm text-primary-500">القيمة المتوقعة: {pendingTask.price_suggested} ريال</p>
            )}
          </div>

          <div className="bg-white border border-slate-300 rounded-xl p-4 space-y-3">
            <p className="text-xs text-slate-400 font-medium uppercase tracking-wide">إقرار وتعهد</p>
            ...
            <div className="bg-slate-100 border border-slate-300 rounded-xl p-3 space-y-2 text-sm">
              ...
              <div className="flex items-center justify-between border-b border-slate-300 pb-2">
                ...
                <span className="text-slate-900 font-medium text-xs">مؤسسة حلول الغد للخدمات الإلكترونية</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-slate-500">رقم الآيبان</span>
            ...
            {pendingTask.price_suggested && (
              <div className="bg-primary-500/10 border border-primary-500/20 rounded-lg px-3 py-2 flex items-center justify-between">
                <span className="text-xs text-slate-500">العمولة المستحقة</span>
                <span className="text-primary-500 font-bold">{(pendingTask.price_suggested * 0.02).toFixed(2)} ريال</span>
              </div>
            )}
        ...
        return (
          <div className="fixed inset-0 bg-slate-900/80 z-50 flex items-center justify-center p-4">
            <div className="bg-slate-50 border border-slate-200 rounded-2xl w-full max-w-md p-6">
              ...
              <p className="text-slate-500 text-sm text-center mb-5">الطلب اكتمل — يرجى تحويل عمولة المنصة</p>
              ...
              <div className="bg-white border border-slate-200 rounded-xl p-4 mb-4 space-y-2">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="text-slate-900 font-medium">{(paymentReminderTask as any).title}</span>
                </div>
                <div className="flex justify-between text-sm border-t border-slate-200 pt-2">
                  <span className="text-slate-500">قيمة الطلب</span>
                  <span className="text-slate-900 font-bold">{price} ريال</span>
                </div>
                <div className="flex justify-between text-sm border-t border-slate-200 pt-2">
                  <span className="text-slate-500">نسبة العمولة</span>
                  <span className="text-primary-500 font-bold">2%</span>
                </div>
                <div className="flex justify-between text-sm border-t border-slate-200 pt-2 bg-primary-500/10 rounded-lg px-3 py-2 -mx-1">
                  <span className="text-slate-700 font-medium">المبلغ المستحق تحويله</span>
                  <span className="text-primary-500 font-black text-base">{commission} ريال</span>
                </div>
              </div>

              {/* IBAN info */}
              <div className="bg-white border border-slate-300 rounded-xl p-4 mb-5 space-y-3">
                <p className="text-xs text-slate-400 font-medium uppercase tracking-wide">معلومات الحساب البنكي</p>
                <div className="flex items-center justify-between border-b border-slate-200 pb-2.5">
                  <span className="text-slate-500 text-sm">البنك</span>
                  <span className="text-slate-900 font-medium text-sm">بنك البلاد</span>
                </div>
                <div className="flex items-center justify-between border-b border-slate-200 pb-2.5">
                  <span className="text-slate-500 text-sm">اسم المستفيد</span>
                  <span className="text-slate-900 font-medium text-xs text-left max-w-[55%]">مؤسسة حلول الغد للخدمات الإلكترونية</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-slate-500 text-sm">رقم الآيبان</span>
                  <button onClick={() => navigator.clipboard.writeText('SA54150009001465965400007')}
                    className="text-primary-500 font-mono text-xs hover:text-primary-300 transition-colors flex items-center gap-1">
              ...
              <p className="text-xs text-slate-400 text-center mb-4">يجب التحويل خلال ٧٢ ساعة من إتمام الطلب وفق الاتفاقية</p>

              <button onClick={() => setPaymentReminderTask(null)}
                className="w-full py-3 rounded-xl bg-primary-500 hover:bg-primary-600 text-slate-900 font-bold text-sm transition-colors">
                فهمت — سأقوم بالتحويل
              </button>
            </div>
          </div>
      ...
      {completingTask && (
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4">
          <div className="bg-slate-50 border border-slate-200 rounded-2xl w-full max-w-md p-5">
            ...
            {proofUrl && (
              <img src={proofUrl} alt="دليل الإنجاز" className="w-full h-40 object-cover rounded-xl mb-4 border border-slate-300" onError={e => (e.currentTarget.style.display='none')} />
            )}

            <div className="flex gap-2">
              <button onClick={() => { setCompletingTask(null); setProofUrl(''); setProofNote('') }}
                className="flex-1 py-2.5 rounded-xl border border-slate-300 text-sm text-slate-500 hover:bg-slate-100 transition-colors">
                إلغاء
              </button>
              <button onClick={submitCompletion} disabled={uploadingProof}
                className="flex-1 py-2.5 rounded-xl bg-secondary-500 hover:bg-secondary-600 text-slate-900 font-bold text-sm transition-colors disabled:opacity-50 flex items-center justify-center gap-2">
          ...
          {TABS.map(({ id, icon: Icon, label, badge }) => (
            <button key={id} onClick={() => setTab(id as Tab)}
              className={`flex items-center gap-1.5 px-3 py-3 text-xs font-medium whitespace-nowrap border-b-2 transition-all ${
                tab === id ? 'border-primary-500 text-primary-500' : 'border-transparent text-slate-400 hover:text-slate-700'
              }`}>
              <Icon size={13} /> {label}
              {badge !== undefined && badge > 0 && (
                <span className="bg-primary-500 text-slate-900 text-[10px] rounded-full w-4 h-4 flex items-center justify-center font-bold">{badge > 9 ? '9+' : badge}</span>
              )}
            </button>
          ))}
        </div>
      ...
      <div className="max-w-4xl mx-auto px-4 py-6">
          ...
          <div className="space-y-5">
            ...
            <div className="bg-white border border-slate-200 rounded-2xl p-5">
              ...
              <div className="grid grid-cols-3 gap-3 text-center">
                <div className="bg-white rounded-xl p-3">
                  <div className="text-lg font-bold text-slate-900">{completedTasks.length}</div>
                  <div className="text-xs text-slate-400">مكتمل</div>
                </div>
                <div className="bg-white rounded-xl p-3">
                  <div className="text-lg font-bold text-slate-900">{activeTasks.length}</div>
                  <div className="text-xs text-slate-400">جاري</div>
                </div>
                <div className="bg-white rounded-xl p-3">
                  <div className="text-lg font-bold text-slate-900">{myTasks.filter(t => t.status === 'cancelled').length}</div>
                  <div className="text-xs text-slate-400">ملغي</div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white border border-slate-200 rounded-2xl p-5">
              <div className="flex items-center justify-between mb-4">
                ...
                <button onClick={() => setTab('my-tasks')} className="text-xs text-primary-500 flex items-center gap-1">الكل <ArrowRight size={12} /></button>
              </div>
              {myTasks.length === 0 ? (
                <p className="text-slate-400 text-sm text-center py-6">ما قبلت أي طلب بعد</p>
              ) : myTasks.slice(0, 5).map(task => (
                <div key={task.id} className="flex items-center justify-between py-3 border-b border-slate-200/50 last:border-0">
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium truncate">{task.title}</p>
                    <p className="text-xs text-slate-400">{new Date(task.created_at).toLocaleDateString('ar-SA')}</p>
                  </div>
                  <div className="flex items-center gap-2 mr-3">
                    {(task.price_final || task.price_suggested) ? (
                      <span className="text-sm font-bold text-primary-500">{task.price_final || task.price_suggested} ر</span>
                    ) : null}
            ...
            <div className="bg-white border border-slate-200 rounded-2xl p-5">
              ...
              <div className="flex flex-wrap gap-2">
                {(workerProfile?.skills || []).map(s => (
                  <span key={s} className="bg-primary-500/10 text-primary-500 border border-primary-500/20 text-xs px-3 py-1.5 rounded-full">{s}</span>
                ))}
              </div>
            </div>
          </div>
        ...
        {tab === 'feed' && (
          <div className="space-y-3">
            ...
            {!workerProfile?.is_online && (
              <div className="flex items-center gap-3 bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-400 mb-2">
            ...
            {feedTasks.length === 0 ? (
              ...
              <div key={task.id} className="bg-white border border-slate-200 rounded-xl p-5 hover:border-slate-300 transition-all">
                <div className="flex items-start justify-between mb-3 gap-3">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                      <span className="text-xs bg-primary-500/10 border border-primary-500/20 text-primary-500 rounded-full px-2 py-0.5">جديد</span>
                  ...
                  {task.price_suggested && (
                    <span className="text-primary-500 font-bold text-sm flex-shrink-0">{task.price_suggested} ر</span>
                  )}
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-xs text-slate-400">{new Date(task.created_at).toLocaleDateString('ar-SA')}</span>
                  <button onClick={() => acceptTask(task)} disabled={accepting === task.id}
                    className="bg-primary-500 text-slate-900 text-sm font-bold px-5 py-1.5 rounded-lg hover:bg-primary-700 transition-colors disabled:opacity-50 flex items-center gap-1">
          ...
          <div className="space-y-3">
            {myTasks.length === 0 ? (
              <div className="text-center py-16 text-slate-400">
                ...
                <button onClick={() => setTab('feed')} className="text-primary-500 text-sm mt-2">تصفح الطلبات المتاحة ←</button>
              </div>
            ) : myTasks.map(task => (
              <div key={task.id} className="bg-white border border-slate-200 rounded-xl p-5">
                <div className="flex items-start justify-between mb-3 gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-slate-400 mt-0.5">{(task as any).profiles?.full_name} · {new Date(task.created_at).toLocaleDateString('ar-SA')}</p>
                  </div>
                  <span className="text-primary-500 font-bold flex-shrink-0">{task.price_final || task.price_suggested || '—'} ر</span>
                </div>
                <div className="flex gap-2">
        ...
        {tab === 'chat' && (
          <div>
            {selectedTask ? (
              <div>
                <button onClick={() => setSelectedTask(null)} className="text-sm text-slate-500 hover:text-slate-900 mb-4 flex items-center gap-1">← رجوع</button>
                <Chat taskId={selectedTask.id} taskTitle={selectedTask.title} />
              </div>
            ) : (
              <div className="space-y-2">
                <p className="text-slate-400 text-sm mb-4">اختر طلب عشان تفتح المحادثة</p>
                {myTasks.filter(t => ['in_progress'].includes(t.status)).length === 0 ? (
                  ...
                  <button key={task.id} onClick={() => setSelectedTask(task)}
                    className="w-full bg-white border border-slate-200 rounded-xl p-4 text-right hover:border-slate-300 transition-all">
                    <p className="font-medium">{task.title}</p>
                    <p className="text-xs text-slate-400 mt-0.5">{(task as any).profiles?.full_name}</p>
                  </button>
                ))}
              </div>
            )}
            ...
            {workerProfile?.schedule && Object.keys(workerProfile.schedule).length > 0 ? (
              <div className="space-y-2">
                {Object.entries(workerProfile.schedule).map(([day, s]: [string, any]) => (
                  <div key={day} className={`flex items-center justify-between rounded-xl px-4 py-3 ${s.active ? 'bg-primary-500/5 border border-primary-500/20' : 'bg-slate-50 border border-slate-200'}`}>
                    <span className={`text-sm font-medium ${s.active ? 'text-slate-900' : 'text-slate-400'}`}>{day}</span>
                    {s.active ? <span className="text-xs text-primary-500">{s.from} — {s.to}</span> : <span className="text-xs text-slate-400">غير متاح</span>}
                  </div>
                ))}
              </div>
            ) : <p className="text-slate-400 text-sm">ما في جدول محدد</p>}
          </div>
        )}
            ...
            <div className="bg-white border border-slate-200 rounded-2xl p-6">
              <div className="flex items-center gap-4 mb-5">
                ...
                <div>
                  ...
                  <p className="text-slate-400 text-sm">{workerProfile?.city} · {workerProfile?.nationality}</p>
                  <div className="flex items-center gap-1 mt-1">
                    <Star size={13} className="text-primary-500 fill-primary-400" />
                    <span className="text-sm font-medium">{workerProfile?.rating ? workerProfile.rating.toFixed(1) : '—'}</span>
                    <span className="text-xs text-slate-400">({completedTasks.length} طلب)</span>
                  </div>
                </div>
              </div>
              {workerProfile?.bio && <p className="text-sm text-slate-500 leading-relaxed border-t border-slate-200 pt-4">{workerProfile.bio}</p>}
            </div>
            ...
            <div className="bg-white border border-slate-200 rounded-2xl p-5">
              ...
              <div className="space-y-2.5">
                {[
                  ...
                  <div key={k} className="flex justify-between items-center border-b border-slate-200/50 pb-2 last:border-0">
