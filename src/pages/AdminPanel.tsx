        ...
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400">أمرني Admin</p>
            </div>
          </div>
          <div className="flex gap-2">
            <button onClick={refresh} disabled={refreshing}
              className="flex items-center gap-2 text-sm text-gray-500 hover:text-gray-900 border border-gray-200 px-3 py-1.5 rounded-lg transition-colors">
              <RefreshCw size={13} className={refreshing ? 'animate-spin' : ''} /> تحديث
            </button>
            <div className="relative group">
              <button className="flex items-center gap-2 text-sm text-gray-500 hover:text-gray-900 border border-gray-200 px-3 py-1.5 rounded-lg transition-colors">
                📥 تصدير
              </button>
              <div className="absolute left-0 top-full mt-1 bg-white border border-gray-300 rounded-xl p-1 hidden group-hover:flex flex-col min-w-[120px] z-10">
                <button onClick={() => exportCSV('users')} className="text-xs px-3 py-2 hover:bg-gray-100 rounded-lg text-right">المستخدمون</button>
                <button onClick={() => exportCSV('tasks')} className="text-xs px-3 py-2 hover:bg-gray-100 rounded-lg text-right">الطلبات</button>
                <button onClick={() => exportCSV('workers')} className="text-xs px-3 py-2 hover:bg-gray-100 rounded-lg text-right">العمال</button>
              </div>
            </div>
          </div>
        </div>
          ...
          {TABS.map(({ id, icon: Icon, label }) => (
            <button key={id} onClick={() => setTab(id as Tab)}
              ...
              {id === 'workers' && stats.pending > 0 && (
                <span className="bg-primary-500 text-gray-900 text-[10px] rounded-full w-4 h-4 flex items-center justify-center font-bold">{stats.pending}</span>
              )}
              {id === 'conversations' && stats.disputed > 0 && (
                <span className="bg-red-500 text-gray-900 text-[10px] rounded-full w-4 h-4 flex items-center justify-center font-bold">{stats.disputed}</span>
              )}
            </button>
          ))}
        </div>
      ...
      <div className="max-w-6xl mx-auto px-4 py-6">
        ...
        {tab === 'overview' && (
          <div className="space-y-6">
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {[
                { label: 'المستخدمون', value: stats.users, color: 'text-gray-900' },
            ...
            {tasks.length > 0 && (
              <div className="grid sm:grid-cols-2 gap-4">
                <div className="bg-white border border-gray-200 rounded-2xl p-5">
                  <h3 className="font-semibold mb-4 text-sm text-gray-500">📍 أكثر المدن نشاطاً</h3>
                ...
                <div className="bg-white border border-gray-200 rounded-2xl p-5">
                  <h3 className="font-semibold mb-4 text-sm text-gray-500">📂 أكثر التصنيفات</h3>
                ...
                <div className="space-y-2">
                  {workers.filter(w => !w.is_approved).map(w => (
                    <div key={w.id} className="bg-white border border-primary-800/30 rounded-2xl p-5 space-y-4">
                      ...
                      <div className="flex items-center justify-between gap-3">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-gray-400 mt-0.5">{w.city} · {w.nationality}</p>
                          </div>
                        </div>
                        <div className="flex gap-2">
                      ...
                      {(w.skills || []).length > 0 && (
                        <div>
                          <p className="text-xs text-gray-400 mb-2">المهارات</p>
                          <div className="flex flex-wrap gap-1.5">
                            {w.skills.map(s => (
                              <span key={s} className="text-xs bg-gray-100 text-gray-700 px-2.5 py-1 rounded-full border border-gray-300">{s}</span>
                            ))}
                          </div>
                        </div>
                      )}
                      ...
                      {w.bio && (
                        <div className="bg-gray-100 rounded-xl p-3">
                          <p className="text-xs text-gray-400 mb-1">النبذة</p>
                          <p className="text-sm text-gray-700">{w.bio}</p>
                        </div>
                      )}
                      ...
                      {w.id_image_url && (
                        <div>
                          <p className="text-xs text-gray-400 mb-2">📷 صورة الهوية</p>
                          <a href={w.id_image_url} target="_blank" rel="noreferrer">
                            <img src={w.id_image_url} alt="ID" className="w-full max-h-48 object-contain rounded-xl border border-gray-300 hover:border-primary-500/50 transition-colors" />
                          </a>
                          <p className="text-xs text-gray-400 mt-1">اضغط على الصورة لتكبيرها</p>
                        </div>
                      )}
                    </div>
                  ))}
        ...
        {tab === 'workers' && (
          <div className="space-y-3">
            {workers.length === 0 ? (
              <p className="text-center text-gray-400 py-12">ما في عمال مسجلين</p>
            ) : workers.map(w => (
              <div key={w.id} className={`bg-white border rounded-2xl p-5 ${!w.is_approved ? 'border-primary-800/30' : 'border-gray-200'}`}>
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-400">{w.city} · {w.nationality} · هوية: {w.id_verified ? '✓' : '✗'}</p>
                  ...
                  <div className="flex gap-2 flex-shrink-0">
                    {!w.is_approved ? (
                      ...
                      <button onClick={() => rejectWorker(w.user_id)}
                        className="px-3 py-1.5 bg-gray-100 text-gray-500 text-xs rounded-lg border border-gray-300">
                        سحب الموافقة
                      </button>
                    )}
                  </div>
                ...
                <div className="flex items-start justify-between gap-3">
                  ...
                  <div className="flex gap-1.5 flex-shrink-0">
                    <button onClick={() => { setSelectedTask(task); setTab('conversations') }}
                      className="flex items-center gap-1 px-2.5 py-1.5 bg-gray-100 text-gray-700 text-xs rounded-lg">
                      <Eye size={11} /> محادثة
                    </button>
        ...
        {tab === 'conversations' && (
          <div className="grid lg:grid-cols-3 gap-4">
            <div className="lg:col-span-1 space-y-2">
              <p className="text-xs text-gray-400 mb-3">اختر طلباً لمشاهدة المحادثة</p>
              {tasks.filter(t => t.status === 'in_progress' || t.status === 'disputed').map(task => (
                <button key={task.id} onClick={() => setSelectedTask(task)}
                  className={`w-full text-right p-3 rounded-xl border transition-all ${
                    selectedTask?.id === task.id ? 'border-primary-500/40 bg-primary-500/5' : 'border-gray-200 bg-white hover:border-gray-300'
                  }`}>
                  <div className="flex items-center gap-2 mb-1">
                  ...
                  <p className="text-xs text-gray-400">{task.status === 'disputed' ? 'نزاع' : 'جاري'}</p>
                </button>
              ))}
              {tasks.filter(t => ['in_progress','disputed'].includes(t.status)).length === 0 && (
                <p className="text-gray-400 text-sm text-center py-8">ما في محادثات نشطة</p>
              )}
            </div>
            <div className="lg:col-span-2">
              {selectedTask ? (
                ...
                <div className="flex items-center justify-center h-80 border border-gray-200 rounded-2xl bg-white text-gray-400">
              ...
              <thead>
                <tr className="border-b border-gray-200">
                  {['الاسم', 'الدور', 'البريد', 'تاريخ التسجيل'].map(h => (
                    <th key={h} className="text-right text-xs text-gray-400 font-medium px-4 py-3">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {users.map(u => (
                  <tr key={u.id} className="border-b border-gray-200/50 hover:bg-gray-100/10 transition-colors">
                    ...
                    <td className="px-4 py-3 text-sm text-gray-400">{new Date(u.created_at).toLocaleDateString('ar-SA')}</td>
                  </tr>
                ))}
              </tbody>
            </table>
