  ...
  return (
    <div className="min-h-screen bg-gray-50 pt-14 px-4 py-6">
      <div className="max-w-xl mx-auto">
        <button onClick={() => navigate('dashboard')} className="flex items-center gap-1.5 text-gray-400 hover:text-gray-900 text-sm mb-4 transition-colors">
        ...
        <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden flex flex-col" style={{ height: '70vh' }}>
          {/* Header */}
          <div className="px-4 py-3 border-b border-gray-200 bg-gray-100/50 flex items-center gap-3">
            ...
            {msgs.map((m, i) => (
              <div key={i} className={`flex items-start gap-2.5 ${m.role === 'user' ? 'flex-row-reverse' : ''}`}>
                <div className={`w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 ${
                  m.role === 'assistant' ? 'bg-primary-500/10 border border-primary-500/20' : 'bg-gray-100'
                }`}>
                  {m.role === 'assistant' ? <Bot size={14} className="text-primary-500" /> : <User size={14} className="text-gray-500" />}
                </div>
                <div className={`max-w-[80%] rounded-2xl px-4 py-2.5 text-sm leading-relaxed ${
                  m.role === 'user' ? 'bg-primary-500 text-gray-900 rounded-tr-sm' : 'bg-gray-100 text-gray-900 rounded-tl-sm'
                }`}>
                  {m.content}
                </div>
              </div>
