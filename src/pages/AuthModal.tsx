...
export function AuthModal() {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-gray-900/50 backdrop-blur-sm">
      <div className="w-full max-w-sm bg-white border border-gray-200 shadow-xl rounded-2xl p-6 relative">
        <button onClick={closeAuth} className="absolute top-4 left-4 text-gray-400 hover:text-gray-900">
          <X size={18} />
        </button>
          ...
          {(['login', 'signup'] as const).map(t => (
            <button key={t} onClick={() => setTab(t)}
              className={`flex-1 py-2 text-sm font-medium rounded-lg transition-all ${
                tab === t ? 'bg-primary-500 text-gray-900' : 'text-gray-500 hover:text-gray-900'
              }`}>
        ...
        <div className="space-y-3">
          {tab === 'signup' && (
            <>
              ...
              {/* Role selection */}
              <div>
                <label className="block text-xs text-gray-400 mb-2">أنا</label>
                  ...
                  {[
                    ...
                    <button key={v} onClick={() => setRole(v as any)}
                      className={`text-right p-3 rounded-xl border transition-all ${
                        role === v ? 'border-primary-500 bg-primary-500/10' : 'border-gray-200 hover:border-gray-300'
                      }`}>
          ...
          {error === '__email_confirm__' ? (
            <div className="bg-secondary-500/10 border border-secondary-500/20 rounded-xl p-3 text-center">
              ...
              <p className="text-gray-500 text-xs mb-2">أرسلنا رابط التأكيد على <span className="text-gray-900">{form.email}</span></p>
              {resendDone ? (
            ...
            <>
              ...
              <button onClick={submit} disabled={loading}
                className="w-full bg-primary-500 text-gray-900 font-bold py-2.5 rounded-xl hover:bg-primary-400 transition-colors disabled:opacity-50 flex items-center justify-center gap-2">
