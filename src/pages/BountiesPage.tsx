      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        <div className="flex items-start justify-between mb-8">
          <div>
            ...
            <p className="text-gray-400">طلبات حية من عملاء ينتظرون — اقبل وابدأ</p>
          </div>
          <button onClick={() => navigate('dashboard')}
            className="flex items-center gap-2 bg-primary-500 text-gray-900 font-bold px-4 py-2 rounded-xl text-sm hover:bg-primary-400 transition-colors">
        ...
        {loading ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-gray-400">ما في طلبات متاحة الآن</p>
            <p className="text-xs text-gray-400 mt-2">اشترك عشان توصلك إشعارات بالطلبات الجديدة</p>
          </div>
        ) : (
          <div className="space-y-3">
            {tasks.map(task => (
              <div key={task.id} className="bg-white border border-gray-200 hover:border-gray-300 rounded-2xl p-5 transition-all">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-2 flex-wrap">
                      ...
                      <span className="text-xs bg-gray-100 text-gray-500 rounded-full px-2 py-0.5">{task.category}</span>
                  ...
                  <button onClick={() => applyTask(task)} disabled={applying === task.id || applied.has(task.id)}
                    className="bg-primary-500 hover:bg-primary-400 disabled:opacity-50 text-gray-900 font-bold px-5 py-2.5 rounded-xl text-sm transition-colors flex-shrink-0 flex items-center gap-1.5">
